...
# Assingment 3
# GRU neural network to predict Google stock prices (Nextday without volume) - Hyperparameter tuning
####################################
...
if not os.path.exists('results/hyperparameter_tuning_nextday_novolume.csv'):

    # test different window sizes
    for window_size in window_size_array:

        # We will get the data using a moving window approach
        x_full, y_full = utils.moving_window_nextday(train_test[['Open','High','Low','Close']].values,window_size)
    ...
    df_tuning.to_csv('results/hyperparameter_tuning_nextday_novolume.csv', index = False)
    print("Hyperparameter tuning finished")

else:
    # In case the tuning was already performed, read the csv from the results folder
    df_tuning= pd.read_csv('results/hyperparameter_tuning_nextday_novolume.csv')
...
df_train.to_csv('results/train_loss_nextday_novolume' + '.csv', index = False)
print("Finished!")
