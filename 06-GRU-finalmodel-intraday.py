...
# Assingment 3
# GRU neural network to predict Google stock prices (Intraday) - Final model (tuned)
####################################
...
# Create a single vector with all the predicted data
predicted = np.vstack((pred_train.cpu().data.numpy(),pred_test.cpu().data.numpy()))

predicted_df = pd.DataFrame(predicted,columns=['High_pred','Low_pred','Close_pred'])
...
actual = train_test[['High','Low','Close']]
...
df_pred.to_csv('results/predicted_values_finalmodel_intraday.csv', index = False)

# Print the RMSE of the train data
print('Train RMSE (Low)= ', utils.rmse(predicted_df.iloc[:len(test),0].values,actual.iloc[:len(test),0].values))
print('Train RMSE (High)= ', utils.rmse(predicted_df.iloc[:len(test),1].values,actual.iloc[:len(test),1].values))
print('Train RMSE (Close)= ', utils.rmse(predicted_df.iloc[:len(test),2].values,actual.iloc[:len(test),2].values))

# Print the RMSE of the test data
print('Test RMSE (Low)= ', utils.rmse(predicted_df.iloc[len(test):,0].values,actual.iloc[len(test):,0].values))
print('Test RMSE (High)= ', utils.rmse(predicted_df.iloc[len(test):,1].values,actual.iloc[len(test):,1].values))
print('Test RMSE (Close)= ', utils.rmse(predicted_df.iloc[len(test):,2].values,actual.iloc[len(test):,2].values))
