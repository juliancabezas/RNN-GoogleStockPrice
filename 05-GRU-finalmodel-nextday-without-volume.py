...
# Assingment 3
# GRU neural network to predict Google stock prices (Nextday without volume)- Final model (tuned)
####################################
...
predicted_df = pd.DataFrame(predicted,columns=['Open_pred','High_pred','Low_pred','Close_pred'])
...
actual = train_test[['Open','High','Low','Close']]
...
df_pred.to_csv('results/predicted_values_finalmodel_nextday_without_volume.csv', index = False)
